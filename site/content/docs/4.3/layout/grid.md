...
Classes are built from the `$gutters` Sass map which is inherited from the `$spacers` Sass map. Each value in the Sass map generates classes for gutters in vertical, horizontal or both ways.
